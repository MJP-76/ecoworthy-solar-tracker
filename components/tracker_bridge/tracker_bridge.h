...
/* ESP-NOW C API -- platform-specific include.
 * ESP8266 NONOS SDK needs extern "C" to avoid C++ name-mangling issues.
 * ESP32 provides esp_now.h as a proper C++-compatible header. */
...
class TrackerBridge : public Component, public uart::UARTDevice {
 public:
  ...
  void setup() override {
    ...
    if (mesh_enabled_) {
      /* Defer mesh setup by 5 seconds to ensure WiFi is fully initialized
       * before calling ESP-NOW functions.  Prevents crash on boot. */
      this->set_timeout("mesh_init", 5000, [this]() {
        ...
        if (test_broadcast_) {
          this->set_interval("mesh_test", 5000, [this]() {
            ...
            mesh_tx_(99, test, 2);
            ESP_LOGI(TAG, "test broadcast sent (ctr=%" PRIu32 ")", tx_counter_);
          });
        }
      });
    }
  ...
  /* Public wrapper for sending commands to STC (plain string + newline) */
  void send_command(const char *cmd) {
    for (const char *p = cmd; *p; p++) this->write_byte((uint8_t)*p);
    this->write_byte('\n');
  }
 ...
 protected:
  ...
  void mesh_setup_() {
     ...
     * Use eFuse MAC which doesn't require WiFi to be initialized. */
#if defined(ARDUINO_ARCH_ESP32)
...
#endif
    ESP_LOGI(TAG, "Mesh enabled: channel=%u name=%s epoch=%" PRIu16 " has_wind_sensor=%s",
    ...
    this->set_interval("mesh_broadcasts", 5000, [this]() {
      ...
      if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) this->mesh_tx_gateway_hb_();
#elif defined(ARDUINO_ARCH_ESP8266)
  ...
  /* ---- recv_cb_: ESP-NOW receive callback entry point ----
   ...
   * ESP32 ESP-IDF 5.x uses esp_now_recv_info* as first parameter. */
#if defined(ARDUINO_ARCH_ESP32)
  static void recv_cb_(const esp_now_recv_info *info, const uint8_t *data, int len) {
    if (TrackerBridge::instance_)
      TrackerBridge::instance_->mesh_rx_(info->src_addr, data, (size_t)len);
  }
#elif defined(ARDUINO_ARCH_ESP8266)
...
#if defined(ARDUINO_ARCH_ESP32)
  void mesh_rx_(const uint8_t * /*mac*/, const uint8_t *data, size_t len) {
#elif defined(ARDUINO_ARCH_ESP8266)
  void mesh_rx_(uint8_t * /*mac*/, uint8_t *data, uint8_t len) {
    ...
    if (sit != peer_last_session_.end()) {
      ...
      if (!advanced) {
        ESP_LOGD(TAG, "replay drop name=%.32s epoch=%" PRIu16 " ctr=%" PRIu32 " last=(%" PRIu16 ",%" PRIu32 ")",
